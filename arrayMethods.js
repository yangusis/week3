...
const postTaxPrices = prices.map(taxes);
...
function redCb(sum, num) {
  return sum - num
}
const totalPopulation = populations.reduce(redCb)
...
function monFilter(cond) {
  return cond.CP > 200
}
const myStrongest = monstersInYourPocket.filter(monFilter)
...
function totalTax(cond) {
  return cond.price + cond.price * cond.tax
}
const totalOrders = orders.map(totalTax)
...
// couldn't get this one to work, just throws the error "cannot read properties of undefined, which I assume the object defines the owner part as undefined for some reason"
function isBob(cond) {
  if(cond.owner === "Bob")
  return cond.price
}
const totalBob = purchases.reduce(isBob)
