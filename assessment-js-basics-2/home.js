...
let greetUser = (name) => { return 'Welcome back, ' + name }
...
function canWeDeliver(zipCode) {
    if(deliveryAreaZipCodes.includes(zipCode)) {
        return "You're in our deliery zone!"
    } else {
        return "Sorry, we can't deliver to that address"
    }
}
console.log(canWeDeliver(852205))
...
deals[0].title = deals[0].title.replace('15', '10')      // learned these string functions just create a new string, don't replace existing ones
console.log(deals[0].title)    // odd interactions, if I specify the 0th index in previous code it will print undefined using deals.title, but if I don't specify in the previous code it will allow me to print it
...
deals[1].desc = deals[1].desc.replace('March', 'April').trim()      // no real way to see if this works in vscode, as it lets you highlight the entire terminal. Unsure if I have to call this separate from the replace method
