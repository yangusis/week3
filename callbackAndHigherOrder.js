...
function cb2(uniqArr) {
  console.log('The new names array with all the duplicate itmes removed is', uniqArr)
}
...
function uniq(arr, cb) {
  ...
  for(i = 0; i < arr.length; i++) {
    for(q = i + 1; q < arr.length; q++) {
      if(arr[i] === arr[q]) {               // something going wrong on this line, I've printed the values side-by-side and it's not recognizing they're equal. not sure if because its checking location in memory, index, or what
...
// the challenge was trying to read and understand what this is even asking for. Didn't work for me :/
